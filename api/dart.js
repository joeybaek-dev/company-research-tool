...
export default async function handler(req, res) {
  ...
  const { action, corp_code, bsns_year } = req.query;
  ...
  try {
    ...
    return res.status(400).json({ error: "action 파라미터 필요 (company/finance/list/multifinance/empstatus)" });
